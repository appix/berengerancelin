 ...
 *
 * @method     sfGuardUserQuery leftJoinProfil($relationAlias = null) Adds a LEFT JOIN clause to the query using the Profil relation
 * @method     sfGuardUserQuery rightJoinProfil($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Profil relation
 * @method     sfGuardUserQuery innerJoinProfil($relationAlias = null) Adds a INNER JOIN clause to the query using the Profil relation
 *
...
abstract class BasesfGuardUserQuery extends ModelCriteria
{
	...
	/**
	 * Filter the query by a related Profil object
	 *
	 * @param     Profil $profil  the related object to use as filter
	 * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
	 *
	 * @return    sfGuardUserQuery The current query, for fluid interface
	 */
	public function filterByProfil($profil, $comparison = null)
	{
		if ($profil instanceof Profil) {
			return $this
				->addUsingAlias(sfGuardUserPeer::ID, $profil->getUserId(), $comparison);
		} elseif ($profil instanceof PropelCollection) {
			return $this
				->useProfilQuery()
				->filterByPrimaryKeys($profil->getPrimaryKeys())
				->endUse();
		} else {
			throw new PropelException('filterByProfil() only accepts arguments of type Profil or PropelCollection');
		}
	}

	/**
	 * Adds a JOIN clause to the query using the Profil relation
	 *
	 ...
	 * @return    sfGuardUserQuery The current query, for fluid interface
	 */
	public function joinProfil($relationAlias = null, $joinType = Criteria::INNER_JOIN)
	{
		$tableMap = $this->getTableMap();
		$relationMap = $tableMap->getRelation('Profil');
		...
		if($relationAlias) {
			...
			$this->addJoinObject($join, 'Profil');
		}
	...
	/**
	 * Use the Profil relation Profil object
	 *
	 * @see       useQuery()
	 *
	 * @param     string $relationAlias optional alias for the relation,
	 ...
	 * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
	 *
	 * @return    ProfilQuery A secondary query class using the current class as primary query
	 */
	public function useProfilQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
	{
		return $this
			->joinProfil($relationAlias, $joinType)
			->useQuery($relationAlias ? $relationAlias : 'Profil', 'ProfilQuery');
	}
