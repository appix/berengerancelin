...
abstract class BasesfGuardUser extends BaseObject  implements Persistent
{
	...
	/**
	 * @var        array Profil[] Collection to store aggregation of Profil objects.
	 */
	protected $collProfils;
	 ...
	 * @var		array
	 */
	protected $profilsScheduledForDeletion = null;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->profilsScheduledForDeletion !== null) {
				if (!$this->profilsScheduledForDeletion->isEmpty()) {
					ProfilQuery::create()
						->filterByPrimaryKeys($this->profilsScheduledForDeletion->getPrimaryKeys(false))
						->delete($con);
					$this->profilsScheduledForDeletion = null;
				}
			}

			if ($this->collProfils !== null) {
				foreach ($this->collProfils as $referrerFK) {
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
	{
		...
		if ($includeForeignObjects) {
			if (null !== $this->collProfils) {
				$result['Profils'] = $this->collProfils->toArray(null, true, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
			}
	...
	public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
	{
		...
		if ($deepCopy && !$this->startCopy) {
			...
			foreach ($this->getProfils() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addProfil($relObj->copy($deepCopy));
				}
			}
	...
	{
		if ('Profil' == $relationName) {
			return $this->initProfils();
		}
	...
	/**
	 * Clears out the collProfils collection
	 *
	 ...
	 * @see        addProfils()
	 */
	public function clearProfils()
	{
		$this->collProfils = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collProfils collection.
	 *
	 * By default this just sets the collProfils collection to an empty array (like clearcollProfils());
	 ...
	 * @return     void
	 */
	public function initProfils($overrideExisting = true)
	{
		if (null !== $this->collProfils && !$overrideExisting) {
			return;
		}
		$this->collProfils = new PropelObjectCollection();
		$this->collProfils->setModel('Profil');
	}

	/**
	 * Gets an array of Profil objects which contain a foreign key that references this object.
	 *
	 ...
	 * @return     PropelCollection|array Profil[] List of Profil objects
	 * @throws     PropelException
	 */
	public function getProfils($criteria = null, PropelPDO $con = null)
	{
		if(null === $this->collProfils || null !== $criteria) {
			if ($this->isNew() && null === $this->collProfils) {
				// return empty collection
				$this->initProfils();
			} else {
				$collProfils = ProfilQuery::create(null, $criteria)
					->filterBysfGuardUser($this)
					->find($con);
				if (null !== $criteria) {
					return $collProfils;
				}
				$this->collProfils = $collProfils;
			}
		}
		return $this->collProfils;
	}

	/**
	 * Sets a collection of Profil objects related by a one-to-many relationship
	 * to the current object.
	 * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
	 * and new objects from the given Propel collection.
	 *
	 * @param      PropelCollection $profils A Propel collection.
	 * @param      PropelPDO $con Optional connection object
	 */
	public function setProfils(PropelCollection $profils, PropelPDO $con = null)
	{
		$this->profilsScheduledForDeletion = $this->getProfils(new Criteria(), $con)->diff($profils);

		foreach ($profils as $profil) {
			// Fix issue with collection modified by reference
			if ($profil->isNew()) {
				$profil->setsfGuardUser($this);
			}
			$this->addProfil($profil);
		}

		$this->collProfils = $profils;
	}

	/**
	 * Returns the number of related Profil objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related Profil objects.
	 * @throws     PropelException
	 */
	public function countProfils(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if(null === $this->collProfils || null !== $criteria) {
			if ($this->isNew() && null === $this->collProfils) {
				return 0;
			} else {
				$query = ProfilQuery::create(null, $criteria);
				if($distinct) {
					$query->distinct();
				}
				return $query
			...
			return count($this->collProfils);
		}
	}

	/**
	 * Method called to associate a Profil object to this object
	 * through the Profil foreign key attribute.
	 *
	 * @param      Profil $l Profil
	 * @return     sfGuardUser The current object (for fluent API support)
	 */
	public function addProfil(Profil $l)
	{
		if ($this->collProfils === null) {
			$this->initProfils();
		}
		if (!$this->collProfils->contains($l)) { // only add it if the **same** object is not already associated
			$this->doAddProfil($l);
		}
	...
	/**
	 * @param	Profil $profil The profil object to add.
	 */
	protected function doAddProfil($profil)
	{
		$this->collProfils[]= $profil;
		$profil->setsfGuardUser($this);
	}
	...
	public function clearAllReferences($deep = false)
	{
		...
		if ($this->collProfils instanceof PropelCollection) {
			$this->collProfils->clearIterator();
		}
		$this->collProfils = null;
