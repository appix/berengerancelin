...
abstract class BasePage extends BaseObject  implements Persistent
{
	...
	public function setMenuIndex($v)
	{
		...
		if ($this->menu_index !== $v) {
			$this->menu_index = $v;
			$this->modifiedColumns[] = PagePeer::MENU_INDEX;
		}
	...
	public function setPublished($v)
	{
		...
		if ($this->published !== $v) {
			$this->published = $v;
			$this->modifiedColumns[] = PagePeer::PUBLISHED;
		}
	...
	public function setHomepage($v)
	{
		...
		if ($this->homepage !== $v) {
			$this->homepage = $v;
			$this->modifiedColumns[] = PagePeer::HOMEPAGE;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->menu_index = ($row[$startcol + 3] !== null) ? (int) $row[$startcol + 3] : null;
			$this->content = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			$this->language = ($row[$startcol + 5] !== null) ? (string) $row[$startcol + 5] : null;
			$this->published = ($row[$startcol + 6] !== null) ? (boolean) $row[$startcol + 6] : null;
			$this->homepage = ($row[$startcol + 7] !== null) ? (boolean) $row[$startcol + 7] : null;
			$this->resetModified();
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(PagePeer::MENU_INDEX)) {
			$modifiedColumns[':p' . $index++]  = '`MENU_INDEX`';
		}
		if ($this->isColumnModified(PagePeer::CONTENT)) {
			$modifiedColumns[':p' . $index++]  = '`CONTENT`';
		}
		if ($this->isColumnModified(PagePeer::LANGUAGE)) {
			$modifiedColumns[':p' . $index++]  = '`LANGUAGE`';
		}
		if ($this->isColumnModified(PagePeer::PUBLISHED)) {
			$modifiedColumns[':p' . $index++]  = '`PUBLISHED`';
		}
		if ($this->isColumnModified(PagePeer::HOMEPAGE)) {
			...
			foreach ($modifiedColumns as $identifier => $columnName) {
				switch ($columnName) {
					...
					case '`SLUG`':
						...
						break;
					case '`MENU_INDEX`':
						$stmt->bindValue($identifier, $this->menu_index, PDO::PARAM_INT);
						break;
					case '`CONTENT`':
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[3], $arr)) $this->setMenuIndex($arr[$keys[3]]);
		...
		if (array_key_exists($keys[7], $arr)) $this->setHomepage($arr[$keys[7]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(PagePeer::MENU_INDEX)) $criteria->add(PagePeer::MENU_INDEX, $this->menu_index);
		...
		if ($this->isColumnModified(PagePeer::PUBLISHED)) $criteria->add(PagePeer::PUBLISHED, $this->published);
		if ($this->isColumnModified(PagePeer::HOMEPAGE)) $criteria->add(PagePeer::HOMEPAGE, $this->homepage);
